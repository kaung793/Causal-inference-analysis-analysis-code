...
needed <- c("subject_id", "fluid_balance_l", "iap12_next", "wsacs_grade", "iap_current_z", "apache_ii_z", "age_z", "sex", "etiology", "creatinine_z", "map_z")
z <- droplevels(d[complete.cases(d[needed]), , drop = FALSE])
fixed_rhs <- "fluid_balance_l + iap_current_z + apache_ii_z + age_z + sex + etiology + creatinine_z + map_z"
...
gc <- summary(gee)$coefficients["fluid_balance_l", ]
rows <- data.frame(
...
b <- unname(coef(po)["fluid_balance_l"]); se <- unname(sqrt(diag(po_v))["fluid_balance_l"])
...
pb <- unname(coef(partial)["fluid_balance_l"])
...
for (i in seq_len(cfg$bootstrap_reps)) {
  ...
  partial_boot[i] <- tryCatch({
    ...
    unname(coef(fit)["fluid_balance_l"])
